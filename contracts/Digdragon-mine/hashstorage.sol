...
contract DigDragonPowerStorage is Ownable {    
    ...
    function setHashPowerOf(uint256 _tokenId, uint256 _newHashPower) public onlyOwner  {
        require(_newHashPower > 0, 'setHashPowerOf: new hash power must be grater than 0');
        tokenIdToHashPower[_tokenId] = _newHashPower; 
    }

    function setHashPowerOfBatch(uint256[] memory _tokenIds, uint256[] memory _newHashPower) public onlyOwner {
        require(_tokenIds.length == _newHashPower.length, "setHashPowerOfBatch: invalid input value");
